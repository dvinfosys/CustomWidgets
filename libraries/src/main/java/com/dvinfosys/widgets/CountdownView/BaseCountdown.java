package com.dvinfosys.widgets.CountdownView;
...
public class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, Constants.sp2px(mContext, 12));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, Constants.sp2px(mContext, 12));
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = Constants.dp2px(mContext, DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowDay) {
            String tempDay = Constants.formatNum(mDay);
        ...
        if (isShowHour) {
            String tempHour = Constants.formatNum(mHour);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(Constants.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            canvas.drawText(Constants.formatNum(mHour), mHourLeft + curTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(Constants.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(Constants.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(Constants.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = Constants.sp2px(mContext, textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = Constants.sp2px(mContext, textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = Constants.dp2px(mContext, suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = Constants.dp2px(mContext, suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = Constants.dp2px(mContext, suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = Constants.dp2px(mContext, suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = Constants.dp2px(mContext, suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = Constants.dp2px(mContext, suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = Constants.dp2px(mContext, suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = Constants.dp2px(mContext, suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = Constants.dp2px(mContext, suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = Constants.dp2px(mContext, suffixMillisecondMarginL);
            isRef = true;
        }
